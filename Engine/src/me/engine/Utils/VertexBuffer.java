...
public class VertexBuffer {
	...
	public void createBuffer(float[] data,int id,int size) {
		if(vbo[id][0]!=-1) {
			Engine.log.warning("Tried to create already existing buffer");
			updateBuffer(data,id,size);
			return;
		}
		if(data.length%size!=0) {
			Engine.log.warning(()->"Array incorrect size "+(data.length));
			return;
		}
	...
	public void updateBuffer(float[] data,int id,int size) {
		if(staticdata) {
			Engine.log.severe("Tried to update an static Buffer");
			return;
		}
		if(data.length%size!=0) {
			Engine.log.warning(()->"Array incorrect size "+(data.length));
